...
(function () {
  ...
  function findTimeCardIndex(storyCards){
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      const k = c.key ?? c.keys ?? '';
      const t = c.title ?? c.name ?? '';
      if (k === KEY_TIME || t === TITLE_TIME) return i;
    }
    return -1;
  }
...
Minutes per action: ${rate}`;
    ...
    const list=Array.isArray(storyCards)?storyCards:[]; // dedupe any duplicates (keep first)
    try{ if(typeof removeStoryCard==='function'){ const hits=[]; for(let i=0;i<list.length;i++){const c=list[i]; if(!c) continue; const k=c.key??c.keys??''; const t=c.title??c.name??''; if(k===KEY_TIME||t===TITLE_TIME) hits.push(i);} if(hits.length>1){ hits.sort((a,b)=>b-a).slice(0,hits.length-1).forEach(i=>{ try{ removeStoryCard(i); }catch{} }); } } }catch{} const idx=(reg.idx>=0)?reg.idx:findTimeCardIndex(list);
